...
//   public readonly yColor: number;
//   public readonly xColor: number;
...
const DataExplorer = React.memo(() => {
  ...
  function groupDataByKey(dataArray, DataItem, key) {
    const groupedData = {};
  ...
  useEffect(() => {
    ...
    if (!createSurface) {
      const chartCreate = SciChartSurface.create(
        divElementId,
        {
          theme:
      ...
      setCreateSurface(chartCreate);
    } else {
      createSurface.then(({ sciChartSurface, wasmContext }) => {
        ...
        surfaceRangex ? sciChartSurface.xAxes.add(
          new DateTimeNumericAxis(wasmContext, {
            visibleRange: new NumberRange(parseFloat(surfaceRangex.min), parseFloat(surfaceRangex.max)),
            axisTitle: "Timestamp",
        ...
        !filteredData.length && sciChartSurface.yAxes.add(
          new NumericAxis(wasmContext, {
            axisAlignment: EAxisAlignment.Left,
            autoRange: checkedItems?.AutoScaleAlways ? EAutoRange.Always : EAutoRange.Once,
        ...
        checkedItems?.LayoutDefault ? filteredData && filteredData.forEach((item, i) => surfaceRange ? sciChartSurface.yAxes.add(
          new NumericAxis(wasmContext, {
            axisTitle: `${item.DataType}-${item.Units}`,
            id: `${i + 1}`,
            visibleRange: new NumberRange(parseFloat(surfaceRange.min), parseFloat(surfaceRange.max)),
            autoRange: checkedItems?.AutoScaleAlways ? EAutoRange.Always : EAutoRange.Once,
            // growBy: new NumberRange(0.1, 0.1),
            // visibleRangeSizeLimit: new NumberRange(0, checkedItems?.autoRange ? 1000000000 : maxPoints > 1 ? maxPoints * 2 : 1),
            drawMajorGridLines: checkedItems?.showGrid || false,
            drawMinorGridLines: checkedItems?.showGrid || false,
            drawMajorTickLines: checkedItems?.showGrid || false,
            axisAlignment: i % 2 == 0 ? EAxisAlignment.Right : EAxisAlignment.Left,
          })
          ...
          :
          filteredData && filteredData.forEach((item, i) => item.DataStreams.map((items, index) =>
            sciChartSurface.yAxes.add(
              new NumericAxis(wasmContext, {
                axisTitle: `${items}`,
                axisTitleStyle: {
                  fontSize: filteredData.length < 2 ? 18 : filteredData.length < 3 ? 14 : 9,
        ...
        // const thresholdPalette = new thresholdPaletteProvider(4, appTheme.MutedOrange, 200, appTheme.VividTeal);
        ...
        if (filteredData.length > 0 && checkedItems?.displayMarkers) {
          ...
          if (settings.Markers.length == NewMarkers.length) {
            ...
            settings.Markers && settings.Markers.map((item) => {
              ...
              if (DeleteMarkers[item.Marker_Set_Name]) {
                ...
                item.Markers.map((marker, i) => {
                  ...
                  if (MarketInputValue.hasOwnProperty(nameWithGroup)) {
                    ...
                    const inputDateAndTime = inputValue.date + "T" + inputValue.time
                    ...
                    const verticalLineAnnotation = new VerticalLineAnnotation({
                      ...
                      axisLabelFill: selectedColor[nameWithGroup] || MarkerColor,
                      axisLabelStroke: appTheme.ForegroundColor,
                      labelPlacement: ELabelPlacement.TopRight,
                      labelValue: item.Marker_Set_Name + "-" + dynamicPropertyName,
                      onDrag: args => {
                        onAnnotationDrag(verticalLineAnnotation);
                        // thresholdPalette.xThresholdValue = verticalLineAnnotation.x1;
          ...
          if (settings.MarkerPlace) {
            settings.MarkerPlace && settings.MarkerPlace.map((item) => {
              const MarkerColor = stringToColor(item.Marker_Set_Name)
                  ...
                  if (MarketInputValue.hasOwnProperty(nameWithGroup)) {
                    ...
                    const verticalLineAnnotation2 = new VerticalLineAnnotation({
                      ...
                      onDrag: args => {
                        ...
                        // thresholdPalette.xThresholdValue = verticalLineAnnotation2.x1;
                        sciChartSurface.invalidateElement();
                      },
                    ...
                    const verticalLineAnnotation2 = new VerticalLineAnnotation({
                      x1: Number(defaultXValue) + Number(minTimestamp),
        ...
        // adding the Xvalues and Yvalues based on CVS data
        filteredData &&
        ...
        const verticallyStacked = [new YAxisDragModifier(),
        new XAxisDragModifier(),
        new RubberBandXyZoomModifier({
          xyDirection: EXyDirection.XDirection, executeOn: EExecuteOn.MouseRightButton,
          easingFunction: easing.outExpo,

        }),
        new MouseWheelZoomModifier({ xyDirection: EXyDirection.YDirection }),
        new ZoomExtentsModifier(),]
        const LayoutDefault = [new ZoomPanModifier({
  ...
  // Post request when template marker removed 
    ...
    <PageRoot >
      ...
      <MainContainer  >
        ...
        <LeftChartContent >
          ...
          <CustomTabPanel value={TabValue} index={0} className="tab-pannel">
            <Box id="element-to-capture" className="" >
              <Box className="chart-wrapper">
                ...
                <div className="chart-main-wrapper" style={{ display: "flex", flexDirection: "column", height: "94%", marginTop: '10px' }}>
                  <div
          ...
          <CustomTabPanel value={TabValue} index={1} className="tab-pannel">
            <Box id="element-to-capture-2" className="tabData" >
              <div>
          ...
          <CustomTabPanel value={TabValue} index={2} className="tab-pannel">
            <div id="element-to-capture-3" className="tabTags nopan nodrag">
              <div>
                <Snackbar
                  anchorOrigin={{ vertical: "bottom", horizontal: "right" }}
                  open={Submitsuccess} autoHideDuration={6000} onClose={handleClose}>
                ...
                <DataInforMation>
                  <Box marginTop={1}>
                    <Box display={"flex"} gap={1} alignItems={"center"}>
                      ...
                      />
                      <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"displayMarkers"}>
                        Display markers
                      </Box>
                    </Box>
                  </Box>
                      ...
                      {Markers &&
                        Markers.map((item) =>
                          ...
                          item.Markers && item.Markers.length ? <div>
                            <Box display="flex" margin="0" marginY={"10px"} justifyContent="space-between" alignItems={'center'} >
          ...
          <CustomTabPanel value={TabValue} index={3} className="tab-pannel">
            <div id="element-to-capture-3" className="tabSetting nopan nodrag">
              <Box>
                <Snackbar
                  anchorOrigin={{ vertical: "bottom", horizontal: "right" }}
                  open={Submitsuccess} autoHideDuration={6000} onClose={handleClose}>
                ...
                <Box maxWidth="300px">
                  ...
                  <GroupChecks>
                    ...
                    />
                    <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"LayoutDefaults"}>{"Default"}</Box>
                    <input
                      name="Layout"
                      type="radio"
                      id={"LayoutStacked"}
                      ...
                      onChange={() => handleCheckboxChange("LayoutDefault")}
                    />
                    <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"LayoutStacked"}>{"Stacked"}</Box>
                    <p></p>
                  </GroupChecks>
                  {/* Auto Range  */}
                  <GroupChecks>
                    ...
                    />
                    <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"AutoRangeOnce"}>{"Once"}</Box>
                    <input
                      name="AutoScale"
                      type="radio"
                      id={"AutoRangeAlways"}
                      ...
                      onChange={() => handleCheckboxChange("AutoScaleAlways")}
                    />
                    <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"AutoRangeAlways"}>{"Always"}</Box>
                    <p></p>
                  </GroupChecks>
                  ...
                  <GroupChecks>
                    ...
                    />
                    <Box component="label" fontSize={{ xs: "14px", md: "16px" }} htmlFor={"showGrid"}>{"Show grid"}</Box>
                  </GroupChecks>
                  {/* change chart Theme  */}
                  <Box marginTop={2}>
                    <FormControl fullWidth>
